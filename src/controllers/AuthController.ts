...
class AuthController {
	...
	public POST_signup = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		...
		try {
			const signUpUserData = await this.authService.signupMagic(userData, false);
	...
	public POST_signup_magic = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		try {
			const userData = req.body;
	...
	public POST_login = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		const userData = req.body;
		try {
	...
	public POST_login_session = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		try {
	...
	public PUT_password = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		try {
			const data = req.body;
			const schema = Joi.object({
				username: Joi.string().required(),
			...
			if (schema.validate(data).error) {
				const error = schema.validate(data).error;
				return respondHTTPError(res, 400, error ? error.message : "Invalid data");
			}
			let userUpdated: User;
			try {
				userUpdated = await this.authService.updatePassword(data.username, data.password);
			} catch (error) {
				if (error instanceof NoDataError) {
	...
	public PUT_update = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		try {
			const data = req.body;
			const schema = Joi.object({
...
const respondHTTPError = (res: Response, statusCode: number, errorMessage?: string) => {
	if (errorMessage) {
